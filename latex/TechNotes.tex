...
The final condition as well as the boundary constrains are defined below:
\begin{itemize}
...
\item Upside constrains due to callability by the CB issuer when $t\in \Omega_{call}$:
\begin{eqnarray}
U \le \max(B_{call}, C_rS)\\
V = 0 \text{, if } U \ge B_{call}
\end{eqnarray}

\item Downside constrains due to putability by the investor when $t \in \Omega_{put}$:
\begin{eqnarray}
U \ge B_{put} \\
...
The PSOR iteration is used to solve the problem of $\mathbf{Ax} = \mathbf{b}$ with constrains, where $\mathbf{A} = (\mathbf{I} + \theta M)$ and $ \mathbf{b} = (\mathbf{I} - (1-\theta)\mathbf{M})\mathbf{U}^{n+1} + \mathbf{f}\Delta t$. Here we use $\mathbf{I}$ to represent identity matrix, $a_{ij}$ to represent the $i$th-row, $j$th-column element in matrix $\mathbf{A}$, and $b_i$ to represent the $i$-th element in vector $\mathbf{b}$. For each $x^k_i$ in the solution space at the $k$-th iteration, the recursion formula is:
\begin{equation}
...
After each iteration step, the constrains in section \ref{sec:intro} are applied once to the solution space. In specific, the upside and downside constrains are applied as follows:
\begin{itemize}
\item
